...
dataset_description <- "Wordbank is an open database of children's vocabulary development, built on the MacArthur-Bates Communicative Development Inventories (CDIs): parent-report checklists of the words a child produces (and, on infant forms, understands). Researchers around the world contribute their CDI administrations; Wordbank archives them in a common schema. This dataset is the canonical, versioned release behind wordbank.stanford.edu and the wordbankr R package (https://github.com/langcog/wordbankr), which reads it directly and takes a version argument for reproducible analyses.

WHERE THE DATA ARE

item_responses is the core table: one row per administration x item (about 64 million rows) with the raw value and its recodes produces / understands.

administrations has one row per child completing a form (age, vocabulary totals, dataset). It joins to item_responses on data_id, to children (demographics, birth variables) on child_id, and to instruments (language x form, normed age range) on language + form.
