...
nw_words = {
  ...
  for (const lang of nw_selected) {
    const slug = san(lang);
    const fname = `net_${slug}.parquet`;
    if (!registered.has(fname)) {
      const resp = await fetch(`slices/networks/${slug}.parquet`);
      if (!resp.ok) continue;
      await db.registerFileBuffer(fname, new Uint8Array(await resp.arrayBuffer()));
      registered.add(fname);
    }
    const rows = await (await conn).query(
      `SELECT language, item_definition, category, uni_lemma, aoa, embedding
       FROM read_parquet('${fname}')`);
    for (const r of rows.toArray()) {
      const o = r.toJSON();
      all.push({
        language: o.language,
        word: o.item_definition,
        category: o.category,
        uni_lemma: o.uni_lemma,
        aoa: Number(o.aoa),
        vec: Float32Array.from(o.embedding.toArray())
      });
    }
  }
  return all;
...
```{ojs network-plot}
{
  ...
  const svg = d3.create("svg")
    ...
    .attr("style", "max-width:100%; height:auto; border:1px solid #eee; border-radius:5px;");
  ...
  const stats = html`<div style="color:#666;font-size:12px;margin-bottom:4px">
    ${nodes.length} words acquired by ${nw_age} months · ${links.length} links</div>`;
